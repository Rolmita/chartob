...
export default async function UserDB({ children }) {
    ...
    return (
        <div className="user-db">
            <ul className='databases-list' style={{ border: '3px solid #202c59' }} >
                        ...
                        <li key={dbName} style={{ color: 'white', listStyle: 'none' }}>
                            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                                ...
                                <div style={{ width: '300px', display: 'flex', flexDirection: 'row', justifyContent: 'end' }}>
                                    <Link className="button btn-form" href={`/databases/edit-database?database=${dbName}`}>
                                        Edit
                                    </Link>
                                    <Link className="btn-form button" href={`/databases/delete-database?database=${dbName}`}>
                                        Delete
                                    </Link>
                                </div>
                            </div>
