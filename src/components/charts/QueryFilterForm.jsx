    ...
    useEffect(() => {
        ...
        if (filters.length > 0) {
            for (let i = 0; i < filters.length; i++) {
                ...
                (columnTypes[i].type.length > 0
                    && (columnTypes[i].type[0].DATA_TYPE == 'datetime' || columnTypes[i].type[0].DATA_TYPE == 'String')
                    && !regex.test(filters[i].interval.modifiedColumn)
                    && !regex.test(filters[i].interval.time1))
                    ? quote = "'"
                    : quote = ""
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', margin: '10px 0' }}>
            <div className='form-row'>
                <button type='button' className='button btn-form' style={{ margin: '5px 0' }}
                    onClick={() => addFilter()}>Add filter</button>
            </div>
            <div className='form-row columns' style={{ alignItems: 'first baseline', marginRight: '10px' }}>
                {filters.map((filter, index) => (
                    <div key={index} style={{ marginRight: '10px' }} className='column-choose'>
                        {index > 0 && (
                        ...
                        <div>
                            <button type="button" className='button btn-form' style={{ margin: '5px' }}
                                onClick={() => removeFilter(index)}>Delete filter</button>
                        </div>
                    </div>
                ))}
            </div>
